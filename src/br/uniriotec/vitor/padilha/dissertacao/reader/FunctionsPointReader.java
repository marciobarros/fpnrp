package br.uniriotec.vitor.padilha.dissertacao.reader;
...
/**
 * Classe respons�vel por carregar arquivos que representam um sistema 
 * 
...
public class FunctionsPointReader
{
	...
	private DataFunction loadDataFunction(DataModel dataModel, Element xmlDataFunction, DataFunctionType type) throws Exception 
	{
		...
		DataFunction element = new DataFunction(name, type, dataModel.countDataFunctions());
	...
	private RecordType loadRecordType(DataModel dataModel, DataFunction dataFunction, Element xmlRecordType) throws Exception
	{
		...
		RecordType ret = new RecordType(dataFunction, name, dataFunction.countRecordTypes());
	...
	private DataElement loadDataElement(RecordType recordType, Element xmlDataElement) throws Exception
	{
		...
		return new DataElement(name, description, recordType, primaryKey, referencedDataModelElementName, referencedRecordTypeName, hasSemanticMeaning, recordType.countDataElements());
	}
	...
	private TransactionFunction loadTransaction(TransactionModel transactionModel, DataModel dataModel, Element xmlTransaction) throws Exception 
	{
		...
		TransactionFunction transaction = new TransactionFunction(name, type, errorMessages, extraDataElements, transactionModel.countTransactionFunctions());
	...
	private FileReference loadFileReference(TransactionFunction transaction, DataModel dataModel, Element xmlFileReference) throws Exception
	{
		...
		for (Element xmlField : XmlUtils.getElements(xmlFileReference, "det"))
			fileReference.addDataElement(loadFileReferenceDataElement(transaction, fileReference, xmlField));
	 ...
	 */
	private DataElement loadFileReferenceDataElement(TransactionFunction transaction, FileReference fileReference, Element xmlField) throws Exception 
	{
		...
		DataElement dataElement = fileReference.getReferencedRecordType().getDataElementName(name);
